  ...
  const Dashboard = () => {
      ...
      const fetchSimulationData = async () => {
        try {
          ...
          if (params.id) {
            ...
            if (simulationResult && simulationResult._id) {
              try {
                const { data: historyData } = await api.get(`/financial/history/${simulationResult._id}`, {
                  headers: { Authorization: `Bearer ${token}` }
                });
                setFinancialHistory(historyData);
                console.log("Financial history loaded:", historyData);
              } catch (historyError) {
                console.error("Error fetching financial history:", historyError);
              }
            }
          ...
          // Add a direct request to get the user's financial data
          try {
            const { data: userData } = await api.get(`/financial/${userId}`, {
              headers: { Authorization: `Bearer ${token}` }
            });
            
            // Merge user financial data with simulation data to ensure we have investments
            if (userData && simulationResult) {
              simulationResult.userFinancialData = userData;
            }
          } catch (userDataError) {
            console.error("Error fetching user financial data:", userDataError);
          }
    ...
    const behaviorAnalysis = useMemo(() => simulationData?.behaviorAnalysis || {
      spendingPatterns: {
        categories: []
      },
      savingsBehavior: {
    ...
    const financialSnapshot = useMemo(() => {
      const income = simulationData?.financialData?.income || simulationData?.chartData?.incomeVsExpenses?.[0]?.amount || 0;
      const expenses = simulationData?.financialData?.expenses || simulationData?.chartData?.incomeVsExpenses?.[1]?.amount || 0;
      const monthlySavings = income - expenses; // Calculate monthly savings
      
      // Extract investments value properly based on the data structure
      let monthlyInvestments = 0;
      
      // Try to get investments from simulation data directly
      if (simulationData?.financialData?.monthlyInvestments !== undefined) {
        monthlyInvestments = simulationData.financialData.monthlyInvestments;
      } 
      // Check if it's in the currentState
      else if (simulationData?.currentState?.monthlyInvestments !== undefined) {
        monthlyInvestments = simulationData.currentState.monthlyInvestments;
      }
      // Check if it's directly in the simulation object
      else if (simulationData?.monthlyInvestments !== undefined) {
        monthlyInvestments = simulationData.monthlyInvestments;
      }
      // Check if we got user financial data from our additional request
      else if (simulationData?.userFinancialData?.monthlyInvestments !== undefined) {
        monthlyInvestments = simulationData.userFinancialData.monthlyInvestments;
      }
      // Check if it's stored in the investments array (sum them up)
      else if (Array.isArray(simulationData?.financialData?.investments)) {
        monthlyInvestments = simulationData.financialData.investments.reduce((total, inv) => total + Number(inv.amount || 0), 0);
      }
      // Check user financial data investments array as well
      else if (Array.isArray(simulationData?.userFinancialData?.investments)) {
        monthlyInvestments = simulationData.userFinancialData.investments.reduce((total, inv) => total + Number(inv.amount || 0), 0);
      }
      ...
      return {
        ...
        savings: simulationData?.financialData?.savings || simulationData?.chartData?.savingsProgress?.[0]?.amount || 0,
    ...
    const financialSnapshotData = useMemo(() => {
      return financialHistory?.financialSnapshot || {
    ...
    const statCards = [
      ...
      },
      {
        ...
        value: formatter.format(financialSnapshot.expenses),
